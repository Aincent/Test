...
HeartMagicMessage::HeartMagicMessage()
{
	DEF_MSG_QUICK_REQUEST_REG_FUN(HeartMagicMessage, MSG_REQ_C2GM_IMPOSE_HEART_MAGIC);
...
DEF_MSG_QUICK_REQUEST_DEFINE_FUN(HeartMagicMessage,MSG_REQ_C2GM_IMPOSE_HEART_MAGIC)
{
	...
	if(player && player->IsOpenSystem(eStHeartMagic))
	{
		eHeartMagicAdvanceType tempType = (eHeartMagicAdvanceType)req.type();
		if(tempType == eHeart_MagicAdvanceAuto && player->GetVipPart() &&  player->GetVipPart()->GetValueByRight(eVipAutoHeartMagic) == VIP_NOT_HAVE_THIS_RIGHT)
		{
			toClient.set_error(eNotVipRight);
		}
		else
		{
			player->GetHeartMagicWeapon()->AdvanceMagicWeapon(&toClient, req);
		}
	}
	else
		return;

	REQ_MSG_QUICK_ACK_FUN(message, &toClient, MSG_REQ_C2GM_IMPOSE_HEART_MAGIC);
}

/*DEF_MSG_QUICK_REQUEST_DEFINE_FUN(HeartMagicMessage,MSG_REQ_C2GM_FLUSH_MAGIC_SKILL)
{
...
/*DEF_MSG_QUICK_REQUEST_DEFINE_FUN(HeartMagicMessage, MSG_SIM_C2GM_REPLACE_MAGIC_SKILL)
{
	ServerReturn::ServerRetInt toClient;
	ServerReturn::ServerRetInt req;
