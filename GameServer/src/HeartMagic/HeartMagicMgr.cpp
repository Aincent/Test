...
HeartMagicMgr::HeartMagicMgr(Player * p):m_owner(p)
{
	ReInit();
}

HeartMagicMgr::~HeartMagicMgr()
{
	this->ReleaseMagic();
}

void HeartMagicMgr::ReInit()
{
	m_id = -1;
	m_exp = 0;
	m_advanceTime = 0;
...
void HeartMagicMgr::ReleaseMagic()
{
...
int HeartMagicMgr::InitExp(const int &exp, const int &daynum)
{
	if(daynum <=0 || exp<=0)
		return 0;
	...
	const MagicParam& ref = GameParaLoader::GetInstance()->GetHeartParam();
	...
	m_owner->SetDataFlag(eHeartMagic);

	return retexp;
}

void HeartMagicMgr::InitMagicWeapon(const PlayerInfo::MagicWeapon & info)
{
	m_owner->SetInitPartDataFlag(eHeartMagic);

	int64 nowTime = CUtil::GetNowSecond();

	m_id = info.id();
	if(m_id <= 0 && !m_owner->IsOpenSystem(eStHeartMagic))
	{
		m_LevelEvent = RegEvent(m_owner,PLAYER_LEVEL_UP_EVENT_TYPE,this,&HeartMagicMgr::onLevelEvent);//玩家升级
		return;
	}
	...
	{
		const MagicParam& ref = GameParaLoader::GetInstance()->GetHeartParam();
		const int Max_Inter_Day = ref.m_maxInterDay;
	...
	for(int i = 0;i < info.passiveskilllist_size();++i)
	{
		...
		HeartMagicTempPassiveSkill tempSkillInfo;
	...
	HeartMagicWeaponInfo* tempInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id);
	...
	vector<HeartMagicTempPassiveSkill>::iterator tPassizeIt = m_PassiveSkillList.begin();
...
void HeartMagicMgr::SetVitality()
{
	if(HeartMagicLoader::GetInstance()->IsFinalMagic(m_id))
	{
...
//上线计算属性
void HeartMagicMgr::onlineCalculateAtt()
{
	...
	eAttValueType tempValueType = AttributeManage::GetInstance()->getAttValueType(eEachModuleAttHeartMagic);
...
//计算一次战斗力
int HeartMagicMgr::calculateFight()
{
	map<ECharAttrType,int64> tempAttList;
	...
	return m_owner->getFightingManage()->setFighting(eFightHeartMagic,tempResult);
}

void HeartMagicMgr::getAttValue(int id,map<ECharAttrType,int64>& attList)
{
	HeartMagicWeaponInfo* tempData = HeartMagicLoader::GetInstance()->GetMagicWeapon(id);
	if(tempData == NULL)
		return ;

	HeartMagicAttrInfo& tempAtt = tempData->attr[m_owner->getAttManage()->getValue<BYTE>(eCharProfession) - 1];
...
void HeartMagicMgr::SetMagicWeapon(PlayerInfo::MagicWeapon * info)
{
	info->set_id(m_id);
	info->set_exp(m_exp);
...
void HeartMagicMgr::SetMagicWeapon(MagicWeapon::ClientMagicWeapon * info)
{
	eHeartMagicMgrStateEnum state = (eHeartMagicMgrStateEnum)CalNowState();

	SetMagicWeaponState(state, info);

	if(state == e_Heart_MagicMgr_Have)
	{
		if(HeartMagicLoader::GetInstance()->IsFinalMagic(m_id))
			m_exp = 0;
		info->set_exp(m_exp);
...
void HeartMagicMgr::SendToClientMagic()
{
	MagicWeapon::ClientMagicWeapon info;

	SetMagicWeapon(&info);

	QuickMessage::GetInstance()->AddSendMessage(m_owner->GetID(),m_owner->GetChannelID(), &info, MSG_SIM_GM2C_HEART_MAGIC_INFO);
}

void HeartMagicMgr::AdvanceMagicWeapon(MagicWeapon::RetAdvanceMagic * toClient,MagicWeapon::RequestAdvanceMagic& requestMsg)
{
...
//一次升阶扣除
ServerRet HeartMagicMgr::oneAdvanceExp(int& currentID,int& currentExp,vector<int>& passiveSkillList,map<int,short>& resultExpList,
		map<int,int>& itemList,int& money,int& golden,bool isAuto)
{
	HeartMagicWeaponInfo* tempInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(currentID);
	if(!tempInfo)
	{
		return eNull;
	}

	if(HeartMagicLoader::GetInstance()->IsFinalMagic(currentID))
	{
	...
	while(tempExp > 0)
	{
		...
		{
			//生星了
			HeartMagicWeaponInfo* tempNextInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(currentID + 1);
			if(!tempNextInfo)
			{
				tempError = eNull;
				break;
...
//增加锻炼度
void HeartMagicMgr::AddWeaponExp(int exp)
{
	...
	HeartMagicWeaponInfo * mInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id);
	if(!mInfo)
	{
		return;
	}
	...
	{
		HeartMagicWeaponInfo * bInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id + 1);
		if(!bInfo)
		{
			break;
		}
		...
		if(mInfo->advance < bInfo->advance)
		{
			//升阶了
			const MagicParam& ref = GameParaLoader::GetInstance()->GetHeartParam();
			int tempQuality = ref.m_FirstSkillQuality;
		...
		mInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id);
		if(!mInfo)
		{
			break;
		}
...
void HeartMagicMgr::BeginTimer(void * arg)
{
	if(m_beginTimer.IsValid())
	{
		m_beginTimer.SetNull();
...
void HeartMagicMgr::AddMagicWeapon(int id, bool isbroad)
{
	if(m_id == id || m_id > 0)
	{
		return;
	...
	HeartMagicWeaponInfo *mInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(id);
	if(!mInfo)
	{
		return;
	}
...
bool HeartMagicMgr::isMagicSkill(int skillID)
{
	HeartMagicWeaponInfo *mInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id);
	if(mInfo == NULL)
	{
...
void HeartMagicMgr::AttrChange(int beforeID,int currentID)
{
...
int HeartMagicMgr::FlushSkill(int index)
{
	int skillid = -1;
	bool isAuto = index >> 8;
	index = index & 0xff;
...
int HeartMagicMgr::ReplaceSkill(int index)
{
	int tempIndex = -1;
	for(size_t i = 0;i < m_PassiveSkillList.size();++i)
	{
...
void HeartMagicMgr::CountDownMagicWeapon( int needtm)
{
	eHeartMagicMgrStateEnum calenum = CalNowState();

	if(calenum  == e_Heart_MagicMgr_BeginTime || calenum == e_Heart_MagicMgr_InTime)
	{
		if(m_beginTimer.IsValid())
		{
			m_beginTimer.SetNull();
		...
		m_beginTimer = TimerInterface::GetInstance()->RegTimePoint(&HeartMagicMgr::BeginTimer, this, 0, 0, 0, 0, needtm);
...
eHeartMagicMgrStateEnum HeartMagicMgr::CalNowState()
{
	if(m_id > 0)
		return e_Heart_MagicMgr_Have;

	else if(m_tempID > 0)
		return e_Heart_MagicMgr_Temp;

	if(m_tempID == -1)
	{
		if(m_attacktime <= 0)
			return e_Heart_MagicMgr_BeginTime;
		else
			return e_Heart_MagicMgr_InTime;
	}

	return e_Heart_MagicMgr_None;
}

void HeartMagicMgr::SetMagicWeaponState(const eHeartMagicMgrStateEnum &funEnum, MagicWeapon::ClientMagicWeapon* info)
{
	switch(funEnum)
	{
	case e_Heart_MagicMgr_Have:
	{
		info->set_id(m_id);
		break;
	}
	case e_Heart_MagicMgr_Temp:
	{
		info->set_tempid(m_tempID);
		break;
	}
	case e_Heart_MagicMgr_BeginTime:
	{
		const MagicParam& ref = GameParaLoader::GetInstance()->GetHeartParam();
		info->set_begintime(ref.m_magicCountDownTime);
		break;
	}
	case e_Heart_MagicMgr_InTime:
	{
		info->set_begintime(m_needTm);
		break;
	}
	case e_Heart_MagicMgr_None:
	{
		info->set_tempid(m_tempID);
		info->set_id(m_id);
...
void HeartMagicMgr::SetTempID(const int id)
{
...
bool HeartMagicMgr::CanUpdateSumitMagic()
{
	if((m_tempID>0 && !freeFlag)
			|| CalNowState() == e_Heart_MagicMgr_None )
		return true;
...
//获得神兵阶数
BYTE HeartMagicMgr::getMagicOrder()
{
	if(m_id <= 0)
		return 0;
...
bool HeartMagicMgr::IsMagicMaxByLv()
{
	if (HeartMagicLoader::GetInstance()->getMaxID() <= m_id)
	{
		return true;
	}

	HeartMagicWeaponInfo* info = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id + 1);
...
bool HeartMagicMgr::IsCanAdvance()
{
	return 0 == CheckAdvanceCond();
}

bool HeartMagicMgr::IsCanSkillRefresh()
{
	return 0 == CheckSkillRefreshCond();
}

int HeartMagicMgr::CheckAdvanceCond()
{
	...
	HeartMagicWeaponInfo* tempInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(curID);
	if(!tempInfo)
	{
		return eNull;
	}

	if(HeartMagicLoader::GetInstance()->IsFinalMagic(curID))
	{
		return eMagicWeaponMaxLv;
	}
...
int HeartMagicMgr::CheckSkillRefreshCond()
{
	Smart_Ptr<PropBase> stone;
...
bool HeartMagicMgr::onLevelEvent(const EventArgs& e)
{
	if(m_id > 0)
	{
...
void HeartMagicMgr::Advance(MagicWeapon::RetAdvanceMagic * toClient,MagicWeapon::RequestAdvanceMagic& requestMsg)
{
	if(m_id <= 0)
	{
		toClient->set_error(eNotHaveHeartMagic);
		return;
	}
	...
	int tempMaxID = HeartMagicLoader::GetInstance()->getMaxID();

	bool tempIsFirst = true;

	ServerRet tempOneError = eRetSuccess;
	eHeartMagicAdvanceType tempType = (eHeartMagicAdvanceType)requestMsg.type();
	if(tempType == eHeart_MagicAdvanceNormal)
	{
	...
	HeartMagicWeaponInfo* tempCurrentInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(m_id);
	HeartMagicWeaponInfo* tempResultInfo = HeartMagicLoader::GetInstance()->GetMagicWeapon(tempCurrentID);
	if(tempResultInfo != NULL && tempCurrentInfo != NULL && tempCurrentInfo->advance < tempResultInfo->advance)
	{
		...
		{
			ChatMgr::GetInstance()->Marquee(eMarqueeHeartMagic,
											m_owner->GetName(),
	...
	if(beforeID != m_id)
	{
		...
		m_owner->FireEvent(PLAYER_WEAPON_HEARTMAGIC_ADVANCE,e);
	}
	...
	if (IsMagicMaxByLv())
	{
		m_owner->UpdateUIHighLight(eUIHLHandle_UpAndSyn, UIHLType(eUIHLBig_Role, eUIHLSmall_Role_HeartMagicCanAdvance), 1, false);
	}
