...
#ifndef GROUPCARBON_H_
#define GROUPCARBON_H_
...
class CGroupCarbon : public CGameDuplicate
{
public:
	CGroupCarbon(int64 onlyid, int64 charid, int64 paramf, int64 params);
	virtual ~CGroupCarbon();
	...
	void SendHarmRankToClient(Smart_Ptr<Player>& player);

	void UpdateHarmRank(Smart_Ptr<Player>& player, int value);

	void SysHarmRank();

	virtual int SetPlayerInstanceFlops(int64 charid,int flopid);

	void DestoryGroupToWs();

	void AddBuffer(int buffId);

	void ClearBuffer(Smart_Ptr<Player>& player);
private:
	map<int64,vector<int> > m_buffers;
	vector<TeamHarm> m_TeamHarm;	//组队副本伤害总计
	int m_nCount;
};


#endif /* GROUPCARBON_H_ */
