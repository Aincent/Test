...
//			obj.UnLock();
			...
			SendMessageToServer(Request, remote_id, remote_type, channel_id, MSG_SIM_GT2GM_PUSHSERVERID, &meContent);
...
//				obj.UnLock();
			...
			SendMessageToServer(Request, remote_id, remote_type, channel_id, MSG_SIM_DB2GM_PUSHSERVERID, &meContent);
	...
	case eWorldServer:
		{
			...
			SendMessageToServer(Request, remote_id, remote_type, channel_id, MSG_REQ_GM2WS_PUSHSERVERID, &meContent);
...
//		m_managerSvr = new SvrItem();
		...
		SendMessageToServer(Request, remote_id, remote_type, channel_id, MSG_REQ_GM2MS_PUSHSERVERID, &meContent);
...
//	GUARD(CSimLock, objgt, &m_gateLock);
	...
	map<int,Smart_Ptr<SvrItem> >::iterator itWorld = m_worldSvr.find(sim.ret());
	if(itWorld != m_worldSvr.end())
	{
		CSceneMgr::GetInstance()->SendAllMap(message->GetRemoteID(), message->GetRemoteType(), message->GetChannelID(), m_localID, m_localType);
	}
//	objgt.UnLock();
}
