...
/**************随机获取被动技能*************/
int SMagicWeaponInfo::RandSkill(int quality,bool isByQuality)
{
	if (isByQuality)
...
int SMagicWeaponInfo::randSkillByQuality(int quality)
{
	vector<SPassivePorbability> tempSkillList;
	...
	ProbabilityVector<SPassivePorbability> tempHandle(tempSkillList);
...
int SMagicWeaponInfo::randSkillNotByQuality()
{
	ProbabilityVector<SPassivePorbability> tempHandle(m_PassaveProbabilityList);
	short tempIndex = tempHandle.calculation();
	if (tempIndex == INVALID_INDEX)
		return 0;
...
int SMagicLoader::Init(string &file)
{
	string tempMagicStr 	  = file + "ArtifactTypeData.csv";
	string tempProbabilityStr = file + "ArtifactSkill.csv";
	//string tempLuaStr 		  = file + "/MagicConfig.lua";

	map<DWORD,vector<SPassivePorbability> > tempDataList;
	map<DWORD,vector<SPassivePorbability> >::iterator tempIt;
...
void SMagicLoader::loadPassive(string& file,map<DWORD,vector<SPassivePorbability> >& dataList)
{
	CSVLoader loader;
...
void SMagicLoader::loaLua(string& file)
{
...
bool SMagicLoader::isMagicSkill(int skillID)
{
