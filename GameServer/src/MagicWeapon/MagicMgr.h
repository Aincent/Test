...
#include "./MagicLoader/MagicLoader.h"
...
enum eMagicMgrStateEnum
{
	...
	e_MagicMgr_Temp ,
	//应该倒计时
	e_MagicMgr_BeginTime ,
	e_MagicMgr_InTime ,
	//没有
	e_MagicMgr_None ,
};

enum eMagicAdvanceType
{
	eMagicAdvanceNull   = 0,
	eMagicAdvanceNormal = 1,
	eMagicAdvanceAuto   = 2,
};

struct MagicTempPassiveSkill
{
	MagicTempPassiveSkill() : m_Index(0),m_SkillID(0)
	{
...
public:
	...
	//计算 现在 处于 什么 状态
	eMagicMgrStateEnum CalNowState();

	void SetMagicWeaponState(const eMagicMgrStateEnum &funEnum, MagicWeapon::ClientMagicWeapon * info);
...
private:
	...
	//刷新出来的技能
	vector<MagicTempPassiveSkill> m_PassiveSkillList;
	//临时ID
	int m_tempID;
	//开始 计时时刻
	int64 m_attacktime;
